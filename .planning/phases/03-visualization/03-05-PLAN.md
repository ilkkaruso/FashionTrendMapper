...
must_haves:
  truths:
    ...
    - "API endpoint serves trends from database including description (MODAL-02)"
    - "Bubbles render with correct data from API"
    - "Filters work and update visible bubbles"
    - "Clicking bubble opens modal with details and description"
...
<task type="auto">
  <name>Update trend repository to include description</name>
  <files>lib/database/trend-repository.ts</files>
  <action>
    Update getTrendsWithChange to select and return description field (MODAL-02):

    In the supabase query, add `description` to the select:
    ```typescript
    .select(`
      id,
      title,
    ...
    In the return object, include description:
    ```typescript
    return {
    ...
    CRITICAL: description is optional (nullable in database), return undefined if null.
  </action>
  <verify>
    TypeScript compiles lib/database/trend-repository.ts without errors.
    Return type TrendWithHistory includes optional description.
  </verify>
  <done>
    getTrendsWithChange returns description field from database.
    Description is optional (undefined when null).
  </done>
</task>
...
<success_criteria>
1. getTrendsWithChange returns description field from database (MODAL-02)
2. /api/trends endpoint returns trends with change data and description
3. Home page fetches trends from API on mount
4. BubbleChart renders with animated bubbles
5. FilterBar displays search, categories, last updated
6. Search and category filters update visible bubbles
7. Clicking bubble opens TrendModal
8. Modal displays trend details including description, sources, related trends
9. ESC and X button close modal
10. Page responsive on mobile and desktop (ResizeObserver)
11. Loading and error states display correctly
12. All TypeScript compilation passes
13. Page marked 'use client'
</success_criteria>
