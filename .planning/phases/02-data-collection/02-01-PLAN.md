...
Purpose: Establish shared types, caching, and rate limiting utilities that the Google Trends fetcher will depend on.
...
```bash
npm install google-trends-api @upstash/ratelimit @upstash/redis
npm install -D @types/google-trends-api
```

Create `lib/fetchers/types.ts` with shared type definitions:
- `SourceType`: 'google' (single source for v1)
- `RawTrend`: Raw data from source (title, score, source, sourceUrl?, metadata?)
...
Keep types simple - no over-engineering. These will be used by the fetcher and normalizer.
  </action>
  <verify>
`npm ls google-trends-api @upstash/ratelimit @upstash/redis` shows all packages installed.
`npx tsc --noEmit` passes without errors on types.ts.
  </verify>
  <done>
All 3 packages installed. Types file exports RawTrend, NormalizedTrend, TrendWithHistory, FetchResult, SourceType.
  </done>
</task>
...
Rate limiter configured with sliding window for Google (10/10s).
...
<verification>
1. All packages in package.json dependencies: google-trends-api, @upstash/ratelimit, @upstash/redis
...
<success_criteria>
- [ ] google-trends-api, @upstash/ratelimit, @upstash/redis installed
...
- [ ] lib/utils/rate-limiter.ts exports ratelimit, checkRateLimit
